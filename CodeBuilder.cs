...
public class CodeBuilder : IDisposable
{
    ...
    /// Write the given string at the current level of indentation plus a newline
    /// </summary>
    public void WriteTn(params string[] strings)
    {
        Write(strings);
        Write(';');
        NewLine();
    ...
    public void Arrow(params string[] xs)
    {
    ...
    public void Declare(string? type, string? name, string? value = null)
    {
        Write(type);
        Spaces();
        Append(name);
    ...
    public string ToString() => _sb.ToString();
